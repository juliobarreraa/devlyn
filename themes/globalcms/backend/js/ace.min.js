...
/**
 * Funciones que actuan sobre el layout principal y la página en completo.
 * @version: $Revision$
 * @author: $Author$
 * @since   # $Date$
 ...
 */
jQuery(function($) {
	var MAX_ITEMS_CHAT = 2;

	var pattern = new RegExp('admin\/' + gcms.controller + '\/');

	/**
	 * Por cada elemento li comparamos si cumple con la expresión regular, si cumple entonces activamos la pestaña.
	 * Puede haber 0 o más pestañas iluminadas.
	 * @param  integer index Posición del elemento actual
	 * @param  object value Objeto actual
	 */
	$('#sidebar ul.nav.nav-list li').each(function(index, value) {
		//Si coincide se activa
		if(pattern.test($(value).find('a:first-child').attr('href')))
			$(value).addClass('active');
	});

	/**
	 * Se muestran 3 mensajes en el layout principal de mensajes
	 * @param  {Object} t Mensaje datos.
	 */
	socket.on('chat new', function(t) {
		//Listado de mensajes
		var $mlistbadge = $('#message-list-badge');
		//Contador de mensajes
		var $mcount = $mlistbadge.find('a span.badge.badge-success');
		//Colección de mensajes
		var $messagesCollection = $mlistbadge.find('ul.pull-right.dropdown-navbar.dropdown-menu.dropdown-caret.dropdown-closer');

		//Leyenda
		var $legend = $messagesCollection.find('li:first-child');

		//Nuevos mensajes
		var newMessage = parseInt($legend.text().match(/(\d+)/g));

		//Si no tiene un número válido configuramos a 0
		if(isNaN(newMessage))
			newMessage = 0;

		//Actualizamos número de mensajes
		$legend.text(gcms.lang.new_message_count.replace(/\{\{count\}\}/, newMessage + 1));
		$mcount.text(newMessage+1);

		var template = _.template($('#tmpl-layout-chat-item').html());

		$messagesCollection.find('li:first-child').after(template({model: t}));
		//Si ya existen más de 3 mensajes insertamos el elemento y eliminamos el último
		if(newMessage > MAX_ITEMS_CHAT)
			$messagesCollection.find('li:eq(' + (MAX_ITEMS_CHAT + 2) + ')').remove();
	});
});